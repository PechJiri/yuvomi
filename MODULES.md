...
How long that line holds: before an operation under `/api/v1` changes or goes away, it is named as deprecated in the CHANGELOG at least one minor release ahead and keeps working unchanged until then. If an `/api/v2` ever ships, `/api/v1` keeps being served for twelve months after it.
