...
> `2.65.0` (exact version), `2.65` (latest patch of that minor), plus a moving `main`
> tag for the current development state. To pin production to a known-good release,
> set `image: ghcr.io/ulsklyc/yuvomi:2.65.0` in your compose file and bump it
...
>
> ```bash
> cosign verify ghcr.io/ulsklyc/yuvomi:2.65.0 \
...
> image: `docker buildx imagetools inspect ghcr.io/ulsklyc/yuvomi:2.65.0 --format '{{ json .Provenance }}'`.
...
```
yuvomi  | [Yuvomi] Server running on port 3000 | Version 2.65.0
