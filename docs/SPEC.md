...
Eligibility is server-owned and local-only. A series must have a recurrence rule, have `external_source = 'local'`, carry no external UID, object URL, calendar reference, subscription, provider link, or Google, CalDAV, or Outlook outbound target, and have no birthday, name-day, housekeeping, or other generated owner. The structural `is_local_recurring_series` classification is separate from the actor-specific `can_override_occurrence` capability. Occurrence-only and following mutations deliberately use the same row-visibility authorization as the established whole-series PUT/DELETE path: every household member who may edit the visible series may edit one occurrence; there is no extra creator-only tier. Google, Apple/CalDAV, Outlook, ICS subscription/import, generated, and outbound-targeted rows therefore remain on their existing provider or whole-series paths. Apple auto-sync additionally leaves a local master with any linked child or deletion EXDATE local, because its unchanged single-VEVENT payload cannot represent that state; neither the master without its exception nor the child as a standalone UID is uploaded. API capability fields are presentation data; every occurrence mutation repeats the authoritative visibility and structural checks before writing.
