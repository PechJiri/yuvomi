...
Eligibility is server-owned and local-only. A series must have a recurrence rule, have `external_source = 'local'`, carry no external UID, object URL, calendar reference, subscription, provider link, or Google, CalDAV, or Outlook outbound target, and have no birthday, name-day, housekeeping, or other generated owner. The caller must also own the series or be an administrator. Google, Apple/CalDAV, Outlook, ICS subscription/import, generated, and outbound-targeted rows therefore remain on their existing provider or whole-series paths. The API capability field is presentation data; every occurrence mutation repeats the authoritative server check before writing.
