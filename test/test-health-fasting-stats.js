...
test('streak credits target-or-longer fasts using the explicit display-zone completion date', () => {
  const result = fastingStreaks(rows, { today: '2026-09-03', timeZone: 'Europe/Prague' });
  assert.equal(result.current, 2);
  assert.equal(result.longest, 2);
});

test('streak and weekly completion dates use one explicit display zone across records', () => {
  ...
  const kiritimati = { id: 2, start_at: '2026-09-13T06:30:00.000Z', end_at: sameInstant, goal_minutes: 960, start_tzid: 'Pacific/Kiritimati' };
  const timeZone = 'America/Los_Angeles';
  assert.deepEqual(fastingStreaks([prague, kiritimati], { today: '2026-09-13', timeZone }), { current: 1, longest: 1 });
  const series = weeklyFastingSeries([prague, kiritimati], { endDate: '2026-09-13', timeZone });
...
test('summaries count sub-minute records and weekly series distinguishes no record and no goal', () => {
  ...
  const series = weeklyFastingSeries(rows, { endDate: '2026-09-14', timeZone: 'UTC' });
...
test('a very long fast is counted exactly without allocating an entry per day', (context) => {
  ...
  assert.deepEqual(fastingStreaks([long], { today: '2026-09-02', timeZone: 'UTC' }), { current: 73788725, longest: 73788725 });
});
...
test('interval streaks merge overlaps and adjacency but preserve gaps and goal eligibility', () => {
  ...
  assert.deepEqual(fastingStreaks(history, { today: '2026-01-09', timeZone: 'UTC' }), { current: 1, longest: 6 });
  assert.deepEqual(fastingStreaks(history.toReversed(), { today: '2026-01-10', timeZone: 'UTC' }), { current: 0, longest: 6 });
  assert.deepEqual(fastingStreaks(history.slice(0, 4), { today: '2026-01-06', timeZone: 'UTC' }), { current: 6, longest: 6 });
});
...
test('interval calendar arithmetic supports early years, eras and Gregorian leap boundaries', () => {
  ...
  ]) assert.deepEqual(fastingStreaks(dates.map(fast), { today: dates.at(-1), timeZone: 'UTC' }), { current: dates.length, longest: dates.length });
});
...
test('credit lengths retain the 24-hour boundary and equivalent offset instants', () => {
  for (const [end, count] of [['2026-01-02T00:00Z', 1], ['2026-01-02T01:00Z', 2], ['2026-01-03T01:00Z', 3]]) {
    ...
    assert.equal(fastingStreaks([fast], { today: '2026-01-03', timeZone: 'UTC' }).longest, count);
  }
});
...
test('streak credit ceiling uses exact duration beyond a whole-day boundary', () => {
  ...
  assert.deepEqual(fastingStreaks([row], { today: '2026-09-02', timeZone: 'UTC' }), { current: 2, longest: 2 });
});

test('calendar helpers reject implicit UTC calendar defaults', () => {
  assert.throws(() => fastingStreaks([], { today: '2026-09-13' }), /timeZone/);
  assert.throws(() => weeklyFastingSeries([], { endDate: '2026-09-13' }), /timeZone/);
  assert.throws(() => fastingStreaks([], { timeZone: 'UTC' }), /today/);
  assert.throws(() => weeklyFastingSeries([], { timeZone: 'UTC' }), /endDate/);
});
