...
function migratedWithReminderWidening() {
  ...
  database.prepare("UPDATE reminders SET pushed_at = '2026-09-13T12:05:00.000Z' WHERE entity_type = 'subscription'").run();
  database.prepare("UPDATE reminders SET dismissed = 1 WHERE entity_type = 'inventory_item'").run();
  database.prepare("UPDATE reminders SET assigned_from = 1 WHERE entity_type = 'event'").run();
...
test('v219 preserves v218 document reminders, widens reminder entities, and adds opt-in switches', () => {
  ...
  assert.equal(database.prepare("SELECT pushed_at FROM reminders WHERE entity_type = 'subscription'").get().pushed_at,
    '2026-09-13T12:05:00.000Z');
  assert.equal(database.prepare("SELECT dismissed FROM reminders WHERE entity_type = 'inventory_item'").get().dismissed, 1);
  assert.equal(database.prepare("SELECT assigned_from FROM reminders WHERE entity_type = 'event'").get().assigned_from, 1);
